...
use super::dot::{Counter, Dot, SiteId};
...
impl Uid {
    ...
    fn generate_pos(pos1: usize, pos2: usize, level: usize) -> usize {
        let (lo, hi) = if Uid::use_boundary_plus_strategy(level) {
            let lo_bound = pos1 + 1;
...
impl FromStr for Uid {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let vlq = base64::decode_config(s, base64::URL_SAFE_NO_PAD)
            .map_err(|_| Error::DeserializeSequenceUid)?;
        Uid::from_vlq(&vlq)
    }
}
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for UidVisitor {
                ...
                V: SeqAccess<'de>,
            {
                ...
                Ok(Uid::from_vlq(&vec)
                    .map_err(|_| de::Error::missing_field("invalid VLQ value"))?)
            }
...
mod tests {
    ...
    fn test_ord() {
        let uid0 = Uid::min();
        let uid1 = Uid {
            position: big(0b1_00000000000000101001),
            site_id: 8,
            counter: 382,
        };
        let uid2 = Uid {
            position: big(0b1_00000000000101010010),
            site_id: 1,
            counter: 5,
        };
        let uid3 = Uid {
        ...
        let uid4 = Uid {
            position: big(0b1_00000000001011110010),
            site_id: 4,
            counter: 4,
        };
        let uid5 = Uid {
            position: big(0b1_00000000001011111101),
            site_id: 4,
            counter: 4,
        };
        let uid6 = Uid::max();
    ...
    fn test_between_basic() {
        let uid1 = Uid {
            position: big(0b1_01111111111111111110),
            site_id: 1,
            counter: 1,
        };
        let uid2 = Uid {
            position: big(0b1_10000000000000000000),
            site_id: 1,
            counter: 1,
        };
        let uid = Uid::between(&uid1, &uid2, DOT);
    ...
    fn test_between_multi_level() {
        let uid1 = Uid {
            position: big(0b1_11111000000000000000),
            site_id: 1,
            counter: 1,
        };
        let uid2 = Uid {
            position: big(0b1_11111000000000000001),
            site_id: 1,
            counter: 1,
        };
        let uid = Uid::between(&uid1, &uid2, DOT);
    ...
    fn test_between_squeeze() {
        let uid1 = Uid {
            position: big(0b1_11111000000000000000_001101010010101010101_1010101010101010101010),
            site_id: 1,
            counter: 1,
        };
        let uid2 = Uid {
            position: big(0b1_11111000000000000000_001101010010101010111_1010101010101010101010),
            site_id: 1,
            counter: 1,
        };
        let uid = Uid::between(&uid1, &uid2, DOT);
    ...
    fn test_between_equals() {
        let uid1 = Uid {
            position: big(0b1_00110011100000000010),
            site_id: 1,
            counter: 1,
        };
        let uid2 = Uid {
    ...
    fn test_between_first_is_shorter() {
        ...
        let uid2 = Uid {
            position: big(0b1_11111000000000000000_001101010010101010101),
            site_id: 2,
            counter: 1,
        };
        let uid = Uid::between(&uid1, &uid2, DOT);
    ...
    fn test_between_first_is_longer() {
        let uid1 = Uid {
            position: big(0b1_11111000000000000000_001101010010101010110),
            site_id: 1,
            counter: 1,
        };
        let uid2 = Uid {
