...
impl<T: Element> Node<T> {
    ...
    fn get_elt(&self, mut idx: usize) -> (&T, usize) {
        if self.is_leaf() {
            for element in &self.elements {
                if idx < element.element_len() {
                    return (element, idx);
                } else {
                    idx -= element.element_len()
                }
            }
        } else {
    ...
    fn get_idx(&self, id: &T::Id) -> Option<usize> {
        let (contains_element, idx) = match self.elements.binary_search_by(|elt| elt.id().cmp(id)) {
            Ok(idx) => (true, idx),
            Err(idx) => (false, idx),
        };
        ...
        if self.is_leaf() && contains_element {
            ...
            char_idx += self.children[..idx + 1]
                .iter()
                .map(|node| node.len)
                .sum::<usize>();
            Some(char_idx)
        } else {
            char_idx += self.children[..idx]
                .iter()
                .map(|node| node.len)
                .sum::<usize>();
    ...
    fn insert(&mut self, elt: T) -> Result<(), Error> {
        ...
        if self.is_leaf() {
            ...
            if self.children[idx].is_full() {
                ...
                if elt.id() > self.elements[idx].id() {
                    idx += 1
                }
            }
    ...
    fn split_child(&mut self, child_idx: usize) {
        let (median, new_child) = {
            ...
            let children = if child.is_leaf() {
                vec![]
            } else {
                child.children.split_off(B)
            };

            let new_child_len = elements.iter().fold(0, |sum, e| sum + e.element_len())
    ...
    fn last_id(&self) -> T::Id {
        let mut node = self;
        while node.is_internal() {
            node = node.children.last().expect("Child must exist!")
        }
        node.elements
            .last()
            .expect("Element must exist E!")
            .id()
            .to_owned()
    }
}
...
impl<'de, T: Element + Deserialize<'de>> Deserialize<'de> for Tree<T> {
    fn deserialize<D>(deserializer: D) -> Result<Tree<T>, D::Error>
    where
        D: Deserializer<'de>,
    {
